...
import { Model, useSceneLoader, useClick, useScene, useBeforeRender } from 'react-babylonjs';
import { uiStore } from './store/ui';
import { AGENT_PARAMS, MAX_UNITS_COUNT, NAVMESH_PARAMS, SELECTABLES } from './constants';
...
export const Ground = memo(() => {
  ...
  const units = navigationStore(state => state.units);
  ...
  //   console.log(
  //     crowd
  //       ?.getAgentNextTargetPath(agents.current.get(selectedId))
  //       .equalsWithEpsilon(crowd?.getAgentPosition(agents.current.get(selectedId)), 1)
  //   );
  //   console.log(
  //     crowd?.getAgentNextTargetPath(agents.current.get(selectedId)),
  //     crowd?.getAgentPosition(agents.current.get(selectedId))        
  //   );
